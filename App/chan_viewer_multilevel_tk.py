...
class MultiLevelViewerWindow(tk.Toplevel):
    ...
    def init_ui(self):
        ...
        # 买卖点汇总
        ttk.Label(info_frame, text="买卖点汇总", font=("Arial", 11, "bold")).pack(pady=(5, 5))

        self.bsp_text = tk.Text(info_frame, width=32, height=25, font=("Consolas", 9),
                                 bg="#f8f8f8", relief=tk.FLAT, wrap=tk.WORD)
        self.bsp_text.pack(fill=tk.BOTH, expand=True, padx=5)

        # 区间套分析结果
        ttk.Separator(info_frame, orient=tk.HORIZONTAL).pack(fill=tk.X, pady=10)
        ttk.Label(info_frame, text="区间套分析", font=("Arial", 11, "bold")).pack(pady=(0, 5))

        self.nesting_text = tk.Text(info_frame, width=32, height=15, font=("Consolas", 9),
                                     bg="#fff8e8", relief=tk.FLAT, wrap=tk.WORD)
        self.nesting_text.pack(fill=tk.BOTH, expand=True, padx=5, pady=(0, 5))
